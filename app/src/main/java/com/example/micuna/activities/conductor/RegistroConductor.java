...
public class RegistroConductor extends AppCompatActivity {
    ...
    void registerDriver(){
        ...
        final String vehiculo = mTextInputVehiculo.getText().toString();

        if (!name.isEmpty() && !email.isEmpty() && !password.isEmpty() && !placa.isEmpty() && !vehiculo.isEmpty()){
    ...
    void saveUser(String id, String name, String email){
        ...
        /*else if (selectUser.equals("client")){
            mDatabase.child("Users").child("Clients").child(id).setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()){
                        Toast.makeText(RegistroCliente.this, "Registro Cliente EXITOSO", Toast.LENGTH_SHORT).show();
                    }
                    else {
                        Toast.makeText(RegistroCliente.this, "Fallo el Registro Cliente", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
